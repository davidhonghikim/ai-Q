...
def test_image_editing_operations():
    ...
    print("\n=== Testing Image Editing Operations ===")
    
    # Import image editing operations
    try:
        from operations.tools.image_editing import resize_image, apply_filter
        print("✓ Image editing imports successful")
    except ImportError as e:
        print(f"✗ Image editing import failed: {e}")
        return False
    ...
    try:
        ...
        PIL_AVAILABLE = False
        print("⚠ PIL not available, skipping image editing tests")
        return True
    ...
    try:
        # Test resize_image
        result = resize_image(test_image_path, 20, 20)
        if result['success']:
            print("✓ resize_image operation working")
            # Clean up output file
            if 'output_path' in result:
                os.remove(result['output_path'])
        else:
            print(f"✗ resize_image failed: {result}")
            return False
        
        # Test apply_filter
        result = apply_filter(test_image_path, 'grayscale')
        if result['success']:
            print("✓ apply_filter operation working")
            # Clean up output file
            if 'output_path' in result:
                os.remove(result['output_path'])
        else:
            print(f"✗ apply_filter failed: {result}")
            return False
        ...
        print(f"✗ Image editing test error: {e}")
        return False
...
def main():
    ...
    test_results.append(("Image Editing Operations", test_image_editing_operations()))
